...
## 2026-06-04 (Update 11)
- **アバター生成コアロジック実装**: Pythonバックエンド側でTrimeshを用いたベース頭部モデルの生成と、ARKitから送られた顔面メッシュ（OBJ）の簡易オフセット結合（Stitching）アルゴリズムを実装。ダミーの球体から、実際の顔形状を反映したバストアップモデルが出力されるようにアップデート。

## 2026-06-04 (Update 12)
- **自動化（UX改善）**: FastAPI側に /jobs/latest エンドポイントを追加し、Unity側が常に最新のジョブIDをポーリングして自動でロードを開始するように修正。手動でのIDコピペ作業を排除。

## 2026-06-04 (Update 13)
- **アーキテクチャ方針の決定**: アバターの表情（Blendshapes/Morph Targets）を確実に動かせるようにするため、静的なメッシュ結合（Trimesh）から「表情実装済みのベーステンプレートモデルに対して、顔写真テクスチャと輪郭形状をマッピングする方式」への移行を決定。
- **作業の中断**: 確実な保存を行い、次回はこの新アプローチ（テンプレート・マッピング方式）の実装から再開予定。

## 2026-06-26 (Update 14)
- **ドキュメント更新**: README.mdに「今後のロードマップ（表情ブレンドシェイプとテクスチャマッピングの実装方針）」を追記しました。

## 2026-06-26 (Update 15)
- **表情（Blendshapes）フル稼働化エンジン実装**: Trimeshによる静的メッシュ結合を廃止し、を用いた「テンプレート・マッピング方式」のコアロジックを導入。
- **ベースモデルの自動生成**: モーフターゲットを持つ検証用テンプレートGLBを動的生成。
- **パーソナライズ**: iOSから送られた顔の輪郭（Bounding Box）に合わせてテンプレートの頂点をバイナリレベルでスケーリング・変形する処理を実装。

## 2026-06-26 (Update 16)
- **iOS通信バグの修正**: ホスト名での名前解決が失敗し、通信エラーが起きていた問題を修正。現在のIPアドレス(10.18.166.109)に直接通信するようにし、通信エラー時にUIが『送信成功』と誤表示するバグも修正しました。
